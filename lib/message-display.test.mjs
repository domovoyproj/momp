...
test("drops thinking blocks entirely when hideThinkingBlock is set", async () => {
  const { getDisplayableAssistantBlocks, isHiddenAssistantBlock, splitFinalAssistantBlocks } = await loadSubject();
  const message = assistant([
    { type: "thinking", thinking: "work through it" },
  ...
  assert.deepEqual(
    splitFinalAssistantBlocks(message, { hideThinking: true }).processBlocks.map((block) => block.type),
    ["toolCall"],
  );
