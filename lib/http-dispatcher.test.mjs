...
bunOnly("Bun's fetch routes remote hosts through HTTP_PROXY and reaches loopback directly", async (t) => {
  ...
  const script = `
    const { configureHttpDispatcher } = await import(${JSON.stringify(pathToFileURL(resolve(import.meta.dir, "http-dispatcher.ts")).href)});
  ...
  const child = spawn(process.execPath, ["-e", script], {
    env: { ...process.env, HTTP_PROXY: proxy.url, HTTPS_PROXY: proxy.url, http_proxy: proxy.url, https_proxy: proxy.url, NO_PROXY: "", no_proxy: "" },
