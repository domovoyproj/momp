...
/**
 * Same lines the TUI prints for `/goal show`. Numbers are formatted with an
 * explicit "en-US" locale: these strings are English, and the server's default
 * locale would otherwise change the grouping separator (e.g. "1 200" under ru).
 */
export function formatGoalStatus(status: GoalStatusInfo): string {
  const used = status.tokensUsed.toLocaleString("en-US");
  const budget = status.tokenBudget !== undefined
    ? `${used} / ${status.tokenBudget.toLocaleString("en-US")} (${Math.max(0, status.tokenBudget - status.tokensUsed).toLocaleString("en-US")} left)`
    : `${used} (no budget)`;
  return [
...
export class GoalModeController {
  ...
  async #budget(rest: string): Promise<GoalCommandResult> {
    ...
    this.#scheduleContinuation();
    return {
      message: budget === undefined ? "Goal budget cleared." : `Goal budget set to ${budget.toLocaleString("en-US")}.`,
