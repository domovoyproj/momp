...
test("getCachedSessionDetails memoizes results and revalidates on file change", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "omp-cache-test-"));
  ...
  const header = { type: "session", id: "sess-1", timestamp: "2026-01-01T00:00:00.000Z", cwd: dir };
  const msg1 = userEntry("m1", null, "First user message");
  writeFileSync(filePath, `${JSON.stringify(header)}\n${JSON.stringify(msg1)}\n`, "utf8");

  const first = await getCachedSessionDetails(filePath);
  assert.equal(first.entries.length, 1);
  assert.equal(first.context.messages.length, 1);

  // Second call returns cached instance
  const second = await getCachedSessionDetails(filePath);
  assert.equal(first.entries, second.entries);
  assert.equal(first.tree, second.tree);

  // Invalidate cache explicitly
  invalidateParsedSessionCache(filePath);
  const third = await getCachedSessionDetails(filePath);
  assert.notEqual(first, third);
  assert.equal(third.entries.length, 1);

  // Appending to file changes mtime/size, automatically revalidates
  const msg2 = assistantEntry("m2", "m1", "Assistant response");
  writeFileSync(filePath, `${JSON.stringify(header)}\n${JSON.stringify(msg1)}\n${JSON.stringify(msg2)}\n`, "utf8");
  const fourth = await getCachedSessionDetails(filePath);
  assert.equal(fourth.entries.length, 2);
  assert.equal(fourth.context.messages.length, 2);
});
