...
// omp substitutes a header value that names an environment variable (bare
// name, no `$` prefix) with that variable's value.
test("resolves environment-backed headers without an API key", async () => {
  process.env.OMP_WEB_DISCOVERY_TEST_TOKEN = "resolved-token";
  try {
    const auth = await resolveModelDiscoveryAuth("omp-web-header-only-test", {
      baseUrl: "https://example.invalid/v1",
      api: "openai-completions",
      headers: { "X-Discovery-Token": "OMP_WEB_DISCOVERY_TEST_TOKEN" },
    });
    assert.equal(auth.apiKey, undefined);
    assert.deepEqual(auth.headers, { "X-Discovery-Token": "resolved-token" });
  } finally {
    delete process.env.OMP_WEB_DISCOVERY_TEST_TOKEN;
  }
});
