...
test("Windows desktop sessions isolate the browser eval compatibility workaround", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  assert.match(startupSource, /process\.platform === "win32"/);
  assert.match(startupSource, /settings = await settings\.cloneForCwd\(sessionCwd\)/);
  assert.match(startupSource, /settings\.override\("browser\.enabled", false\)/);
});
