...
如果 worktree 里有未提交或未跟踪文件，Git 会拒绝删除。omp-web 会再显示 force remove。force remove 会丢弃这个 checkout 里的未提交文件，只在确定不需要这些改动时使用。
...
Git 有时会保留 prunable worktree 记录。omp-web 会过滤这些记录，不在切换器里显示。
