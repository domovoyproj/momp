...
const PROJECT_ALIASES_STORAGE_KEY = "omp-project-aliases";
...
function saveProjectAliases(aliases: Record<string, string>): void {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(PROJECT_ALIASES_STORAGE_KEY, JSON.stringify(aliases));
  } catch {}
}
...
export function SessionSidebar({ selectedSessionId, optimisticSession, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions, onBackgroundTaskDone }: Props) {
  ...
  const handleRenameProject = (project: string) => {
    ...
    const currentAlias = projectAliases[key] || "";
    const newName = window.prompt(t("sidebar.rename") + " (оставьте пустым для сброса):", currentAlias);
    if (newName !== null) {
  ...
  const projectGroups = projectPaths.flatMap((project) => {
    ...
    const name = projectAliases[key] || segments.at(-1) || project;
